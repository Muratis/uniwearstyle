...
use App\Models\Cataloge_admin\Sweatshirts;
use SleepingOwl\Admin\Model\ModelConfiguration;
AdminSection::registerModel(Sweatshirts::class, function (ModelConfiguration $model) {

	$model->setTitle('Свитшоты');
	$model->setAlias('sweatshirts');
		...
		$display->SetTabs(function ()
		{
			...
			$kpi->paginate(5)
				->setApply( function ($query) {
					...
					$query->where('clothes_type', '=', 'sweatshirt');
				});
	...
	$model->onCreateAndEdit(function () {
		$form = AdminForm::panel()->addBody(
			...
			AdminFormElement::hidden('clothes_type')->setDefaultValue('sweatshirt'),
			AdminFormElement::number('price', 'Цена грн')->required(),
			AdminFormElement::textarea('description', 'Описание товара'),
			AdminFormElement::multiselect('cataloge', 'Выберите размер')->setModelForOptions(new Sizes())->setDisplay('name')->required(),
